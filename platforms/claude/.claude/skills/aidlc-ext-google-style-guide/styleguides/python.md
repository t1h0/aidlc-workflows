...
- Symbols from the following modules are used to support static analysis and
    type checking:
  - [`typing` module](#21912-imports-for-typing)
  - [`collections.abc` module](#21912-imports-for-typing)
...
[Naming](#216-naming) below. Please explain the design reasons why mutable
...
must be named using all caps with underscores. See [Naming](#216-naming)
below.
...
[nested function](#116-lexical-scoping).
...
[getters and setters](#215-getters-and-setters) rules otherwise.
...
([make the attribute public instead](#215-getters-and-setters)). In comparison,
...
[decorator](#117-function-and-method-decorators). Manually implementing a
property descriptor is considered a [power feature](#119-power-features).
...
Decorators are a special case of "top-level code" - see [main](#217-main) for
more discussion.
...
### 1.20 Modern Python: from \_\_future\_\_ imports
...
above; see the [indentation](#24-indentation) section for explanation.

[Docstring](#281-docstrings) summary lines must remain within the 80 character
limit.
...
vertically (see [line length examples](#22-line-length)), or use a hanging
...
[when a type annotation is present](#2194-default-values), *do* use spaces
...
*Args:*:   List each parameter by name. A description should follow the name, and be
...
Public attributes, excluding [properties](#113-properties), should be documented
...
[explicit line continuation](#22-line-length); in this case, the backslash is
...
[exceptions for `typing` and `collections.abc` imports](#21912-imports-for-typing).
...
1. Python future import statements. For example:
    ...
    See [above](#from-__future__-imports) for more information about those.
...
operation is occurring. Alternatively, [properties](#113-properties) may be an
option when simple logic is needed, or refactoring to no longer need getters and
setters.

Getters and setters should follow the [Naming](#216-naming) guidelines, such
...
- single character names, except for specifically allowed cases:
  ...
  - private [type variables](#21910-type-variables) with no constraints (e.g.
        `_T = TypeVar("_T")`, `_P = ParamSpec("_P")`)
  - names that match established notation in a reference paper or algorithm
        (see [Mathematical Notation](#2165-mathematical-notation))
...
#### 2.16.4 Guidelines derived from [Guido](https://en.wikipedia.org/wiki/Guido_van_Rossum)'s Recommendations
...
Try to follow the existing [indentation](#24-indentation) rules.
...
[alias](#2196-type-aliases) for the type. The last resort is to break after the
...
*Annotated Assignments*:   If an internal variable has a type that is hard or impossible to infer,
...
*Type Comments*:   Though you may see them remaining in the codebase (they were necessary
...
[alias](#2196-type-aliases) with a meaningful name, and use the real type name from
...
remember that in many cases [`TypeVar`](#21910-type-variables) might be more
appropriate:
