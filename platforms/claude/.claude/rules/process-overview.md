...
• **Workspace Detection** (always) → **Reverse Engineering** (brownfield only) → **Requirements Analysis** (always, adaptive depth) → **Conditional Phases** (as needed) → **Workflow Planning** (always) → **Code Generation** (always, per-unit) → **Build and Test** (always) → **Plan Review** (always, loops back to Construction if gaps found)

## How It Works:
• **AI analyzes** your request, workspace, and complexity to determine which stages are needed
• **These stages always execute**: Workspace Detection, Requirements Analysis (adaptive depth), Workflow Planning, Code Generation (per-unit), Build and Test, Plan Review
...
flowchart TD
    ...
    style PR fill:#4CAF50,stroke:#1B5E20,stroke-width:3px,color:#fff
...
- Plan Review: Verify implementation against inception plan, loop back if gaps found (ALWAYS)
...
- CONSTRUCTION focuses on "how" plus "build and test" plus "plan review"
